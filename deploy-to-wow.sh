...
#
# What it does:
#   1. Bumps the trailing version segment in BattlegroundWinConditions.toc
#      (9.8.N → 9.8.(N+1)). Done first so a malformed .toc fails fast
#      before anything destructive happens to the live install. The prefix
#      (e.g. 9.8) is preserved — edit it manually for larger releases.
#   2. Wipes /Applications/World of Warcraft/_retail_/Interface/AddOns/BattlegroundWinConditions
...
#   package-addon.sh  — the packaging script
#
# Kept:
#   .toc / .xml / .lua  — addon code
#   core/, interface/, libs/, maps/, predictions/ — addon assets
#   logo.tga          — addon logo

set -euo pipefail

SRC="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEST="/Applications/World of Warcraft/_retail_/Interface/AddOns/BattlegroundWinConditions"
...
# Only the trailing numeric segment is bumped (9.8.N -> 9.8.(N+1)).
# The prefix (9.8) is preserved — bump it manually for larger releases.
...
# Explicit rm guarantees no orphan files survive between deploys.
rm -rf "$DEST"
mkdir -p "$DEST"
